...
namespace icv { namespace core
{
        ...
        {
        //std::cout<<"Go to construct a node manager wtih node number: "<<node_name_map_->size()<<endl;

        long temp=icvTime::time_ns();
        try{
                AllPublisher.resize(0);
                if(node_name_map_->size()>0)
                {
                        for (auto node_pair : *node_name_map_)
                        {
                                //std::cout<<"current node has : "<<node_pair.second->GetOutputPorts().size()<<" publisher(s)."<<endl;
                                ...
                                for(int i=0;i<node_pair.second->GetOutputPorts().size();i++)
                                {
                                       // cout<<"cpkt1"<<endl;
                                        AllPublisher.push_back(node_pair.second->GetOutputPorts()[i]);
                                }
                               
                              
                                //std::cout<<"The number of all publishers is: "<<AllPublisher.size()<<endl;
                        }
                }
        }
        catch(std::exception e){   ICV_LOG_INFO<<"Node Manager Down!"; }
