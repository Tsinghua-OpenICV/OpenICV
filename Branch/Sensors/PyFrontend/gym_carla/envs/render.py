  ...
  def draw_road_map(self, map_surface, carla_world, carla_map, world_to_pixel, world_to_pixel_width):
    ...
    def draw_topology(carla_topology, index):
      ...
      for waypoint in topology:
        ...
        SHOULDER_COLOR = COLOR_BLACK#COLOR_ALUMINIUM_5
        SIDEWALK_COLOR = COLOR_BLACK#COLOR_ALUMINIUM_3
...
class BirdeyeRender(object):
  def __init__(self, world, params):
    ...
    # Create Surfaces - Zwt: this is time-cosuming, optimize it!
  ...
  def render_waypoints(self, surface, waypoints, world_to_pixel):
    ...
    for p in waypoints:
      corners.append(carla.Location(x=p[0],y=p[1]))
    corners = [world_to_pixel(p) for p in corners]
    pygame.draw.lines(surface, color, False, corners, 5)
