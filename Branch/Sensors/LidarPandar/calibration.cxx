...
void Calibration::read(const std::string& calibration_file) 
{
    ...
    if (calibration_file.empty ())
       ...
       std::cout<<"file 1:"<<calibration_file<<std::endl;     
    ...
    std::cout<<"file 2:"<<calibration_file<<std::endl;     

        file_path = boost::filesystem::path("pandar40.csv");
        if (!boost::filesystem::is_regular(file_path))
            std::cout<<"file 3:"<<file_path.string()<<std::endl;     
    ...
    if (!ifs.is_open())
          {
            std::cout<<"file no open not open not open"<<file_path.string()<<std::endl;
    ...
    while (std::getline(ifs, line)) {
        ...
        laser_corrections[line_id].sinVertCorrection = std::sin (elev* M_PI / 180.0);
        laser_corrections[line_id].cosVertCorrection = std::cos (elev* M_PI / 180.0);
    }
}
