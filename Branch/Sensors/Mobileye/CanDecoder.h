...
//////////////////////////////////////////////////////////////////////////
// data in  hexadecimal
inline void printdata(const unsigned char * data, const unsigned long length, const int id)
{
    printf("\ndata = ");
    ...
    if (length>32) {
     ...
     if (length == 32) {
            ...
            *result = (c1 << 24) + (c2 << 16) + (c3 << 8) + c4;
            ...
            c[2] = data[(startBit>>3) + 2] & mask[2];
            c[3] = data[(startBit>>3) + 3] & mask[3];
            *result = ( (c[0]) + (c[1]<<8) + (c[2]<<16) + c[3]<<24) ;
            return true;
        }
}
    ...
    if (length == 32) {
        ...
        *result = (c1 << 24) + (c2 << 16) + (c3 << 8) + c4;
        return true;
            ...
            unsigned long temp = ( (c[0]) + (c[1]<<8) + (c[2]<<16) + c[3]<<24);
            if (temp & getShiftMask32(length-1, 1))
                // do the two's complement to get the signed value if the msb=1
                *result = -1 * ( ( (~temp) + 1 ) & getMask32(length) );
            else
    ...
    if (length == 16) {
        ...
        *result = (MSB << 8) + LSB;
        return true;
            ...
            unsigned char LSB, MSB, maskMSB, maskLSB, nbBitsMSB, nbBitsLSB;
            nbBitsLSB = (startBit & 0x07) + 1;  // the number of bits contained in the MSB
            nbBitsMSB = static_cast<unsigned char>(length - nbBitsMSB);
            maskLSB = getMask8( maskLSB );
            maskMSB = getShiftMask8(8 - maskMSB , maskMSB);
...
inline int mobileyeDecodeCAN(unsigned char *data,unsigned char bigEndian,unsigned int startBit,unsigned int length,unsigned char Signed)
{
    ...
    if(bigEndian) {
        ...
        if(length>8){
            if(Signed){mDecodeToI16(&stemp16,data, startBit, length);value = stemp16;}
            else {mDecodeToUI16(&ustemp16,data, startBit, length);value = ustemp16;}}
        else if(Signed) {mDecodeToI8(&ctemp8,data, startBit, length);value = ctemp8;}
        else {mDecodeToUI8(&uctemp8,data, startBit, length);value = uctemp8;}
    } else {
        if(length>8){
            if(Signed) {iDecodeToI16(&stemp16,data, startBit, length);value = stemp16;}
            else {iDecodeToUI16(&ustemp16,data, startBit, length);value = ustemp16;}}
        else if(Signed) {iDecodeToI8(&ctemp8,data, startBit, length);value = ctemp8;}
        else {iDecodeToUI8(&uctemp8,data, startBit, length);value = uctemp8;}
    }
