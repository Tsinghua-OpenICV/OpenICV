...
void IPM::init( const cv::Size& _origSize, const cv::Size& _dstSize, const std::vector<cv::Point2f>& _origPoints, const std::vector<cv::Point2f>& _dstPoints )
	
{
	//assert( m_origPoints.size() == 4 && m_dstPoints.size() == 4 && "Orig. points and Dst. points must vectors of 4 points" );
	m_origSize = _origSize;
	m_dstSize = _dstSize;
	m_origPoints.assign(_origPoints.begin(), _origPoints.end());
	m_dstPoints.assign(_dstPoints.begin(), _dstPoints.end());
...
/*
LD_COEFF applyHomography(const LD_COEFF& _coeff)
{
	...
	// const double u = m_H.at<double>(0,0) * _point.x + m_H.at<double>(0,1) * _point.y + m_H.at<double>(0,2) * _point.z;
	// const double v = m_H.at<double>(1,0) * _point.x + m_H.at<double>(1,1) * _point.y + m_H.at<double>(1,2) * _point.z;
	// const double s = m_H.at<double>(2,0) * _point.x + m_H.at<double>(2,1) * _point.y + m_H.at<double>(2,2) * _point.z;
	// if ( s != 0 )
	// {
	// 	ret.x = ( u / s );
	// 	ret.y = ( v / s );
