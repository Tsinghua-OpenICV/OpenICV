...
{
   ManufacturerTable.resize(ManufacturerTable.size()+1);
   ManufacturerTable.insert( ManufacturerTable.size(),manufacturer_p );
}

void MANUFACTURER_LIST::Empty( void )
{
   int loop_index = 0;
   int number_of_entries = ManufacturerTable.size();
/*
...
const QString& MANUFACTURER_LIST::Find( const QString& mnemonic )
{
   static QString return_value;
   ...
   */
   ...
   lower_limit = 0;
   upper_limit = ManufacturerTable.size();
   index       = upper_limit / 2;
...
{
   int index = 0;
   int number_of_entries_in_table = ManufacturerTable.size() - 1;
   ...
   while( sorted == FALSE )
   {
      ...
      while( index < number_of_entries_in_table )
      {
         ...
         if ( this_manufacturer->Mnemonic.compare( that_manufacturer->Mnemonic ) > 0 )
         {
			...
			ManufacturerTable.insert(index,that_manufacturer);	
            //ManufacturerTable[ index + 1 ] = this_manufacturer;
			ManufacturerTable.insert(index + 1,this_manufacturer);
...
const QString& MANUFACTURER_LIST::operator [] ( const char *mnemonic )
{
