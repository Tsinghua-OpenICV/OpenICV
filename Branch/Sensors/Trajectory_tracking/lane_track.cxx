...
class LaneTracking: public icvFunction
{
public:
typedef data::icvStructureData<donnees_gps> icvdonnees_gps;
typedef data::icvStructureData<veh_info> icvveh_info;
  LaneTracking(icv_shared_ptr<const icvMetaData> info) : icvFunction(info){
        Register_Sub("donnees_gps");
        Register_Sub("veh_info");
        Register_Pub("wheel_angle");
        Register_Pub("flag_end");
	    };
//           virtual void ConfigurateOutput(std::vector<icvPublisher*>& outputPorts) override {
// 	 CheckDataType<data::icvDoubleData>(outputPorts[0],sizeof(double));
//     CheckDataType<data::icvInt32Data>(outputPorts[1],sizeof(Int32));
...
//         icvPublisher* output_con= inputPorts[0]->GetConnections()[0];
// 	     bool correct;
// 	   if(output_con->RequireDataObject(false))
//              correct= dynamic_cast<icvdonnees_gps*>(output_con->RequireDataObject(false));
//         else
//         {
			
//             output_con->SetDataObject((icvDataObject*)(new icvdonnees_gps));
//             correct= true;
//         }
...
//         icvPublisher* output_con1= inputPorts[1]->GetConnections()[0];
// 	    if(output_con1->RequireDataObject(false))
//         correct= dynamic_cast<icvveh_info*>(output_con1->RequireDataObject(false));
//         else
//         {
			
//             output_con1->SetDataObject((icvDataObject*)(new icvveh_info));
//             correct= true;
//         }
// 		if (!correct)ICV_LOG_TRACE<<"Data type not matched";
// 		else ICV_LOG_TRACE<<"Data type  matched";
...
void chatcallback2(const veh_info& canmsg)//回调函数  
{  
   // ROS_INFO("I heared Speed is:[%f]",canmsg.Speed); 
    ACC_v=canmsg.Speed;
    ACC_v=ACC_v/3.6;
    //ROS_INFO("I heared wheel angle is:[%f]",canmsg.); 
    wheel_angle_vehicle=canmsg.steerAngle;
    vehicle_wheel_dre=canmsg.steerDir; 
    vehicle_wheel_speed=canmsg.steerSpeed; 
...
count_++;
   ...
   // ros::Publisher pubacc=nh.advertise<std_msgs::Int8>("ACC_brake",1000);    //can
    float vehicle_attitude;
    uint8 acc_off;
    veh_info_control vehicle_state;
    
if(count_==0)
{
    ifstream ifile1("x.txt");
    ifstream ifile2("y.txt");
	...
	vehicle_attitude=wheel_angle_degree_next;
	...
	flag=flag+1;
    dataoutlock.lock();
    data::icvDoubleData datatosend;
    datatosend.setvalue(wheel_angle_degree_next);
    icvPublish("wheel_angle",&datatosend);
    data::icvIntData datatosend2;
    datatosend2.setvalue(flag_end);
    icvPublish("flag_end",&datatosend2);
...
ICV_REGISTER_FUNCTION(LaneTracking)
