...
//ofstream ftestangle("angle.txt",ios::app);   //ios::app 从文件末尾开始写，防止丢失文件中原来就有的内容
...
void lon_alt2xyzCallback(const sensor_msgs::NavSatFix::ConstPtr& msg)         //订阅到话题数据并进行转换为xyz
{
 ...
 if (index_i<10)
  ...
  lat[1]=39.9991806041;
  lon[1]=116.330256125;  //meishuguan,yanshi
  //lat[1]=39.9996074401;
  //lon[1]=116.330067118;  //meishuguan,4gewan
  //lat[1]=39.999310494;
  //lon[1]=116.330686504;  //meishuguan,brake
 if (index_i<3)
    {
     a=6378137.0;
     f=298.2572; 
...
{ 
  ...
  while (ros::ok())
     ...
     //ftestangle<<angle_imu<<endl;                       //写入txt中
...
void imu_orientation(double lon_temp_m,double lon_m){
