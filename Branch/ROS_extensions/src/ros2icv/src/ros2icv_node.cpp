...
#include "structureRadar.h"
#include "structureIMU.h"
#include "ros2icv/ivsensorlrrobj.h"
#include "ros2icv/ivsensorsrrobj.h"
#include "ros2icv/radarlrrobject.h"
#include "ros2icv/radarsrrobject.h"

#include "icvMsgPackRecorder.h"
...
void copy_radar_data(
    /* [in] */  const ros2icv::ivsensorlrrobj &msg,
...
void copy_radar_data(
    /* [in] */  const ros2icv::ivsensorsrrobj &msg,
...
void long_range_radar_callback_lrr1(const ros2icv::ivsensorlrrobj &msg)
{
...
void long_range_radar_callback_lrr2(const ros2icv::ivsensorlrrobj &msg)
{
...
void short_range_radar_callback_srr1(const ros2icv::ivsensorsrrobj &msg)
{
...
void short_range_radar_callback_srr2(const ros2icv::ivsensorsrrobj &msg)
{
...
void short_range_radar_callback_srr3(const ros2icv::ivsensorsrrobj &msg)
{
...
void short_range_radar_callback_srr4(const ros2icv::ivsensorsrrobj &msg)
{
...
{
    //use rosrun ros2icv ros2icv_node /TARGET/PATH/ to execute the program 
    // argv[1] - folder_path
    if (argc != 2) {
        printf("usage: ros2icv path\n");
        return 0;
    }
    
    signal(SIGQUIT, signal_handler_fun);   //press ctrl+\ to stop the process

    folder_path = "/home/yining/OpenICV/Branch/ROS_extensions/test_examples";
    
    imagefolderpath_ = boost::filesystem::path(folder_path);
    imagefolderpath_ = imagefolderpath_ / "image";
    pointcloudpath_ = boost::filesystem::path(folder_path);
    pointcloudpath_ = pointcloudpath_ / "pc_hdl";
    mkdir(imagefolderpath_.string().c_str(),S_IRUSR | S_IWUSR | S_IXUSR | S_IRWXG | S_IRWXO);
    mkdir(pointcloudpath_.string().c_str(),S_IRUSR | S_IWUSR | S_IXUSR | S_IRWXG | S_IRWXO);
    init_golbal();
    ros::init(argc, argv, "ros2icv");
    ros::NodeHandle nh;
    std::cout<<"starting to subscribe ros topics"<<endl;
    ...
    // topics:      /Delta_T                               92 msgs    : std_msgs/Float64               
    //          /dsrc_fusion                          490 msgs    : dsrc_msgpack/TrackArray        
    //          /dsrc_vision_localisation             955 msgs    : dsrc_msgpack/VisionLocalisation
    //          /final_fusion                         310 msgs    : ros_fusion/TrackArray          
    //          /gps/fix                             2304 msgs    : sensor_msgs/NavSatFix          
    //          /gps/fix_ublox                         92 msgs    : sensor_msgs/NavSatFix          
    //          /imu/data                            2303 msgs    : sensor_msgs/Imu                
    //          /lrrTopic                             309 msgs    : ivsensorlrr/ivsensorlrrobj     
    //          /usb_cam_left/image_raw/compressed    230 msgs    : sensor_msgs/CompressedImage    
    //          /velodyne_points                      230 msgs    : sensor_msgs/PointCloud2
