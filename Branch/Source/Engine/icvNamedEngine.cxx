        ...
        for (auto key : info->GetKeys())
        {
            ...
            if (node_params->Contains(_keys::typeKey))
            {
               ...
               // ICV_LOG_INFO<<"INFO TEMP pub "<<temp_pub_name->size();
                ...
                for (auto subname_temp: *temp_map)
                   { 
                       // ICV_LOG_INFO<<"INFO TEMP sub "<<temp_map->at(0);

                    if (!subname_temp.second.empty())
                        _input_name_map.emplace(key+_keys::portSplitpoint+subname_temp.second, function->GetInputPort(subname_temp.second));
                   }
