...
void LaneDetectionCV::Execute() 
{
    ...
    time_t time_new;
    icv::data::icvCvMatData srcImage;
    icvSubscribe("first_cam",&srcImage);
    ICV_LOG_INFO<<"go to ckpt1";
    if (!srcImage.is_not_empty()||srcImage.getvalue().cols<640) {
	return;
    }
    ICV_LOG_INFO<<"go to ckpt2";
    cv::Mat Orig_Image=srcImage.getvalue();
    //time_new=srcImage.GetSourceTime();
    InputTaskPair input ;
    ...
    img_resize = Orig_Image(rect);
    //img_resize = srcImage.getvalue();
...
ld_Frame framePub(LaneDetection lane, cv::Mat Xm)
{
    ...
    {	
        ...
        tmp_Coeff.id = lane.v_PolyfitParam.v_ld_coeff[i].global_id;
        tmp_Coeff.d_img = 0 ;
        tmp_Coeff.c_img = Xm.at<float>(0, i * 3 + 2);
        tmp_Coeff.b_img = Xm.at<float>(0, i * 3 + 1);
        tmp_Coeff.a_img = Xm.at<float>(0, i * 3);
        ld_obj.lane_Coeff.push_back(tmp_Coeff);
    }

    ld_obj.header.stamp = icvTime::time_us();
...
int LaneDetectionCV::process()
{
    ...
    while (true)
    {
        ...
        output.first = task.first ;
        output.second.image = result.image;
        output.second.lane = framePub(lane_, result.Xm);
    
        show_queue_.Enqueue(output);        
...
void LaneDetectionCV::display()
{
    ...
    while (true)
    {
        ...
        lane_output_->setvalue(show.second.lane);
        icvPublish("lanedata",lane_output_);
       
        string _name="Lane Detection Result";
        img_data->setvalue(show.second.image);
        icvPublish("lanedetection_result",img_data);
		imshow(_name, show.second.image);
        waitKey(1);
        ...
        t2 = clock() ;
        //ICV_LOG_INFO<< "\n---Lane Detection Mean Process Time: "<<(t2-t1)/1000/frame_count_ <<" ms"<<std::endl ;
