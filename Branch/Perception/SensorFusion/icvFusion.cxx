...
{
public:
    SensorFusion(icv_shared_ptr<const icvMetaData> info) : icvFunction(info) 
    {
        Eigen::Matrix3d m;
        ...
        Register_Pub("fusion_output");
        Register_Pub("fusion_visual");
        Register_Sub("radar_front");
        Register_Sub("radar_rear");
        Register_Sub("radar_side1");
        Register_Sub("radar_side2");
        Register_Sub("radar_side3");
        Register_Sub("radar_side4");
        Register_Sub("first_cam");
        Register_Sub("lanedata");
        Register_Sub("GPS_data");
        Register_Sub("vel_data");
        Register_Sub("IMU_data");
    ...
    void obs_callback_lane_data_online(const ld_Frame& msg){
        ...
        double lane_time_stamp = icvTime::time_s();
    ...
    void obs_callback_camera_bbox2d_online(const BoundingBoxes2d &_objects){
     ...
     void publish_final_fusion()
     {
        ...
        if (_obs_track.size()>0)
        {
            ...
            fusion_output->setvalue(track_array);
            icvPublish("fusion_output",fusion_output);
    ...
    virtual void Execute() override
    {
        ...
        icvSubscribe("radar_front",&lrr1_data);
        icvlrr lrr2_data;
       
        icvSubscribe("radar_rear",&lrr2_data);
		
        icvsrr srr1_data;
        icvSubscribe("radar_side1",&srr1_data);

        icvsrr srr2_data;
        icvSubscribe("radar_side2",&srr2_data);

        icvsrr srr3_data;
        icvSubscribe("radar_side3",&srr3_data);

        icvsrr srr4_data;
        icvSubscribe("radar_side4",&srr4_data);

        icvImu imu_data;
        icvSubscribe("IMU_data",&imu_data);

        icvTwistWithCovarianceStamped gps_data;
        icvSubscribe("vel_data",&gps_data);

        icvNavSatFix imu_fix;
        icvSubscribe("GPS_data",&imu_fix);

        //icvbbox2d cam_data;
        //icvSubscribe("first_cam",&cam_data);

        icvlane lane_data;
        icvSubscribe("lanedata",&lane_data);
        ...
        //  
        //  
         if (lrr1_data.is_not_empty())
         {
            obs_callback_local_front_radar_online(lrr1_data);
           
         }
          if (lrr2_data.is_not_empty())
         {
            obs_callback_local_front_radar_online(lrr2_data);
         }
          if (srr3_data.is_not_empty())
          {
             obs_callback_local_right_front_radar_online(srr3_data);
          }
          if (!srr4_data.is_not_empty())
          {
             obs_callback_local_left_front_radar_online(srr4_data);
          }
         if (gps_data.is_not_empty())
         {
            obs_callback_gps_vel_online(gps_data);
         }
         if (imu_data.is_not_empty())
         {
            obs_callback_imu_data_online(imu_data);
         }

         if (imu_fix.is_not_empty())
         {
            obs_callback_gps_fix_online(imu_fix);
         }
         /*
         if (cam_data.is_not_empty())
        {
            obs_callback_camera_bbox2d_online(cam_data);
            fusion_visual->setvalue(_Engine.img_lane);
            icvPublish("fusion_visual",fusion_visual);
           
        }
        */
        if (lane_data.is_not_empty())
        {
