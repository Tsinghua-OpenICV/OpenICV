...
// 3-double string define
#define sf3u(f1,f2,f3,u)	1.0*(f1)*(u),1.0*(f2)*(u),1.0*(f3)*(u)
#define sf3(f1,f2,f3)		1.0*(f1),1.0*(f2),1.0*(f3)
#define sf3p(f1,f2,f3)		1.0*(f1)/RE,1.0*(f2)/RE,1.0*(f3)
#define sf30				0.0,0.0,0.0
...
public:
	...
	friend CMat3 vxv(const CVect3 &v1, const CVect3 &v2);	// column-vector multiply row-vector
...
public:
	...
	CVect Xk, Zk, Qt, Rt, rts, Xmax, Pmax, Pmin, Pset, Zfd, Zfd0,
