...
{
    ICV_LOG_TRACE << " Darknet OpenICV started";    
...
Darknet::~Darknet(){
    ICV_LOG_TRACE << " Darknet OpenICV End";
}
void Darknet::Execute(){
...
cv::Mat Darknet::Classfier(cv::Mat &image,torch::jit::script::Module module)
{
...
// bool Darknet::inBorder(const cv::Point2f &pt)
// {
...
// double Darknet::distance(cv::Point2f &pt1, cv::Point2f &pt2)
// {
//     //printf("pt1: %f %f pt2: %f %f\n", pt1.x, pt1.y, pt2.x, pt2.y);
//     double dx = pt1.x - pt2.x;
//     double dy = pt1.y - pt2.y;
...
// map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> Darknet::trackImage(double _cur_time, const cv::Mat &_img, const cv::Mat &_img1)
// {
//     TicToc t_r;
//     cur_time = _cur_time;
//     cur_img = _img;
...
// void Darknet::rejectWithF()      //一个去畸变的过程//通过基本矩阵剔除outliers
// {
//     if (cur_pts.size() >= 8)
//     {
...
// void Darknet::readIntrinsicParameter(const vector<string> &calib_file)
// {
...
// void Darknet::showUndistortion(const string &name)
// {
...
// vector<cv::Point2f> Darknet::undistortedPts(vector<cv::Point2f> &pts, camodocal::CameraPtr cam)
// {
...
// vector<cv::Point2f> Darknet::ptsVelocity(vector<int> &ids, vector<cv::Point2f> &pts, 
...
// void Darknet::drawTrack(const cv::Mat &imLeft, const cv::Mat &imRight, 
...
// void Darknet::setPrediction(map<int, Eigen::Vector3d> &predictPts)
// {
//     hasPrediction = true;
//     predict_pts.clear();
...
// void Darknet::removeOutliers(set<int> &removePtsIds)
// {
...
// cv::Mat Darknet::getTrackImage()
// {
//     return imTrack;
// }

// HDMapObservation* Darknet::getDetection(const map<HDMapType, std::string> & detFilePath,torch::jit::script::Module module) {
...
// void Darknet::showDetectionResult(HDMapObservation * hdmapObs) {
//     // draw lamp
