	...
	void serialSubscriber()
	{
		...
		while(true)
		{
			...
			while(ch!='*')
			{
        		...
        		if(msglen==msgSize-1) 
        		{
		          printf("msg out of size : continue\n");
		          break; // no valid msg
        		}
				msg[msglen++]=ch;
				read(fd,&ch,1);
			...
			{ // startch=='#'
				...
				memset(header, 0, sizeof(header));
				strncpy(header,msg,18);
				//printf("%s\n",msg);
				if(strcmp(header,"TRAN#HEADINGA,COM3")==0)
				{
	...
	{	
		...
		while(true)
		{
			...
			while(ch!='*')
			{
        		...
        		if(msglen==msgSize-1) 
        		{
		          printf("D msg out of size : continue\n");
		          break; // no valid msg
        		}
				msg[msglen++]=ch;
				read(fd_g,&ch,1);
			...
			if(startch=='$')
			{
				...
				char *buf = (char *) malloc(strlen(msg) + strlen("$\r\n") + 1);
				sprintf(buf, "%s%s%s", "$", msg, "\r\n");
				//printf("%s",buf);
				//qxwz_rtcm_sendGGAWithGGAString(buf);
