/*
*  code for test maximum steering curveture
*  1. send a steering commande at certain velocity
*  2. record the GPS position in a txt file
*
*  Author: Shengjie Kou
*  Contact me at ksj18@mails.tsinghua.edu.cn for any questions  
*/

#ifndef _SteerCurve_H
#define _SteerCurve_H
...
class SteerCurve: public icvUdpReceiverSource
{
public:

SteerCurve(icv_shared_ptr<const icvMetaData> info) : icvUdpReceiverSource(info){
	...
	icv_thread loop(bind(&SteerCurve::InnerLoop, this));
...
{
	timesync_->Lock();  
    imu_fix = read_Input<icvNavSatFix>(0); 
	count_++;
	if (send_can_)
	{
		int begincount=1000; 		
    ...
    if (send_steer_flag_)
    {
        std::ofstream record("gps_position.txt");
        record << imu_fix.header.stamp << " " <<imu_fix.latitude << " "<<imu_fix.longitude<<" " <<imu_fix.altitude <<std::endl;
        record.close();
    }
...
ICV_REGISTER_FUNCTION(SteerCurve)
