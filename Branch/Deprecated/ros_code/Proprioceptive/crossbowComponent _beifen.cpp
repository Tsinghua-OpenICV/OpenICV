...
ComponentBase::COMPONENT_CONFIGURATION CrossbowComponent::configureComponent(XmlComponentConfig config)
{
    ...
    openPort( config.getProperty("port").toLatin1() );  

    configurePort(config.getProperty("baudrate").toLong(), 
        config.getProperty("bytesize").toUInt(),
        config.getProperty("parity").toShort(),
        config.getProperty("stopbits").toUInt() - 1); 
    verbose_ = config.getProperty("verbose").toInt();
