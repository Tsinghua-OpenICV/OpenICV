...
/** @file
    ...
    Implementation for converting a Pandar40 3D LIDAR PointXYZIR cloud
...
namespace pandar_pointcloud
{
  ...
  RingColors::RingColors(ros::NodeHandle node, ros::NodeHandle private_nh)
  {
    ...
    output_ =
      node.advertise<sensor_msgs::PointCloud2>("pandar_rings", 10);

    // subscribe to PandarScan packets
    input_ =
      node.subscribe("pandar_points", 10,
                     &RingColors::convertPoints, this,
                     ros::TransportHints().tcpNoDelay(true));
  }


  /** @brief Callback for Pandar40 PointXYZRI messages. */
  void
    RingColors::convertPoints(const PPointCloud::ConstPtr &inMsg)
  {
