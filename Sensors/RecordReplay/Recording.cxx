...
public:
    ...
    Recording(icv_shared_ptr<const icvMetaData> info) : icvFunction(info) 
    {
        ...
        icv_thread loop(bind(&Recording::InnerLoop, this));	
	}
    ...
    virtual void Execute(std::vector<icvNodeInput*>& inputPorts, std::vector<icvNodeOutput*>& outputPorts) override
    {
        ...
        if(is_not_empty(0))
        {
            ...
            readdata=read_Input<icv::data::icvCvMatData>(0).get_Ptr();
            currtimestamp[0]=read_Input<icv::data::icvCvMatData>(0).GetSourceTime();
            ...
            else
            {
                if(SyncClock::time_s()-starttime_<recordrange_*60)
                {
                    if((currtimestamp[0]-lasttimestamp[0])>0)//to check new data
                    {
                        ...
                        cv::Mat temp_mat= readdata->getPic();
