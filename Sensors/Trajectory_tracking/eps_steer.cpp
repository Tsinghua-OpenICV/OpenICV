...
class EPSsteer: public icvFunction
{
public:
  EPSsteer(boost::shared_ptr<const icv::icvMetaData> info) :icvFunction(0,0,info){
	    };
...
void messageCallbackacc(const CanFrameTuanLWI_01& acc_msg)
{
...
void messageCallbackcan(const veh_info& can_msg)
{
	//ROS_INFO("ACC speed:[%f]",can_msg->Speed);
	velocity_can=can_msg.Speed;
	//ROS_INFO("yaw rate:[%f]",can_msg.YawRate);
	Wr_can=can_msg.YawRate;
	//ROS_INFO("steer anglevalue:[%f]",can_msg.steerAngle);
	theta_real_canvalue=can_msg.steerAngle;
	//ROS_INFO("steer anglesign:[%d]",can_msg->steerDir);
	theta_real_cansign=can_msg.steerDir;
...
float chatterseek(float kp[][2], float theta,int row)
{
 ...
 virtual void Execute(icvDataObject** inData, icvDataObject** outData) override
   ...
   //前馈部分变量定义
	...
	float Moment_Expect_value;//定义输出期望输入力距的topic信息
	...
	//控制器主程序
		...
		Moment_Expect_value=Moment_Expect1;
		...
		float angle_expect_value;
		float angle_real_value;
		angle_expect_value = theta_expect;
		angle_real_value = theta_real_can;
...
ICV_REGISTER_FUNCTION(EPSsteer)
