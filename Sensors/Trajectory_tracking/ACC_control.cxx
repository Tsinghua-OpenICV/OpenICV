...
//       icvNodeOutput* output_con= inputPorts[1]->GetConnections()[0];
// 	   correct;
...
buff_Output(outputPorts);
		...
		acc_off=read_Input<data::icvInt32Data>(0);

		chatcallback(read_Input<data::icvStructureData<veh_info>>(1));
		count_++ ;
    ...
    float accel_pre;  //unknown,storage last moment accel
			...
			//ICV_LOG_TRACE<<"ms: "<<icv::SyncClock::now_ms().time_since_epoch().count()<<endl;
		//	ICV_LOG_TRACE<<"s"<<icv::SyncClock::now_s().time_since_epoch().count()<<endl;
