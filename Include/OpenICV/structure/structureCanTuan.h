...
typedef struct
{
 ...
 MSGPACK_DEFINE(YawState,axState,ayState,ay,ax,YawToRight);
...
{ 
 ...
 uint8 EPS_StrWhlTorqueSt; //St=State//55|1@1+ (1,0) [0|1] ""  Frontradar,MQB_MFK_2
  MSGPACK_DEFINE(EPSRxHCA_Status,EPS_StrWhlTorque,EPS_StrWhlTorqueDrt,EPS_StrWhlTorqueSt);
...
MSGPACK_DEFINE(BrakePressureState,BrakePressure);
...
MSGPACK_DEFINE(LF_WhlSpdDrt,LR_WhlSpdDrt,RF_WhlSpdDrt,RR_WhlSpdDrt);
...
{ 
 ...
 MSGPACK_DEFINE(ACC_02CRC,ACC_02BZ,ExpectedSpeed,SpacingFactor,NoiseAlarm,SetSpacing,ImageAlarm,ACC02Status);
...
MSGPACK_DEFINE(ACC_06CRC,ACC_06BZ,aLowerDeviation,aExpectedValue,
aUpperDeviation,DclrtGradRmdValue,AclrtGradRmdValue,StartRequest06,ParkingRequest06,ACC06Status);
...
typedef struct CanFrameTouranACC_07
{
 ...
 uint8 aExpValue; //acceleration deceleration expected value
  MSGPACK_DEFINE(ACC_07CRC,ACC_07BZ,ParkingDistance,ParkingRequest07,StartRequest07,aExpValue);
...
typedef struct CanFrameTouranPLA_01
{
 ...
 uint8 PLASignalTxCyclic; //63|1@1+ (1,0) [0|1] "" Vector__XXX
  MSGPACK_DEFINE(PLA_CRC,PLA_BZ,PLABrkRqtSt,PLAExpStrWhlAngle,PLAExpStrWhlAngleDrt, PLARequestStatus,PLABrkTorque,
  PLABrkDeceleration,PLABrkEnable,BrkTrqAndDeceSwt,PLAParking,PLAPrkDistance, PLASignalTxCyclic);
...
MSGPACK_DEFINE(ExpStrWhlTorque,HCATranCycle,ExpStrWhlTorqueDrt,HCA_Status);
...
{ 
 uint8 ACCSignalContinuity; //38|1@1+ (1,0) [0|1] ""  Frontradar
  MSGPACK_DEFINE(ACCSignalContinuity);
...
typedef  struct
{
 ...
 uint8 SpeedState; //55|1@1+ (1,0) [0|1] ""  Frontradar,MQB_MFK_2
   MSGPACK_DEFINE(Speed,ESP_SystemStatus,SpeedState);
...
{ 
 ...
 uint8 EngNeutralTorque; //engine neutral torque//37|1@1+ (1,0) [0|1] "" Vector__XXX
   MSGPACK_DEFINE(aPedalPercent,aPedalPercentSt,ThrottleGradientSize,ThrottleGradientPN,EngNeutralTorque);
...
typedef struct
{
 ...
 float Acceleration; 
 bool ACC_aktif; 
   MSGPACK_DEFINE(PicWarn,AudioWarn,SteeringToRight,SteeringAngle,PLA_actif,TorqueToRight,Torque,HCA_aktif,Acceleration,ACC_aktif);
...
typedef struct
{
 ...
 MSGPACK_DEFINE(LWI_Sensorstatus,LWI_StrWhlAngleSt,LWI_StrWhlAngleSize,LWI_StrWhlAngleDrt,LWI_StrWhlSpeedDrt,LWI_StrWhlSpeedSize);
...
{ 
 ...
 MSGPACK_DEFINE(Stalls,TargetStalls);
}CanFrameGear_11;
//=====================================================
...
{ 
 ...
 float32 EngineSpeed; //16|16@1+ (0.25,0) [0|16383] "Unit_MinutInver"  Frontradar
  MSGPACK_DEFINE(EngineSpeedState,EngineSpeed);
...
{ 
 float32 EngTorqueCoefficient; //12|2@1+ (1,0) [1|3] "Unit_NewtoMeter" Vector__XXX
  MSGPACK_DEFINE(EngTorqueCoefficient);
...
typedef struct CanFrameTouranOBD_01
{
 ...
 float32 aPedalPosition; //40|8@1+ (0.392156862745098,0) [0|100] "Unit_PerCent" Vector__XXX
  MSGPACK_DEFINE(ThtottlePosition,aPedalPosition);
