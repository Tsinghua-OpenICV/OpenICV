...
namespace icv { namespace _impl
{
    ...
    void icvMsgpackRecorder::Record(const icvDataObject* data, const Uint32& source)
    {
        if(first_)
        {
        ...
        if((SyncClock::time_s()-start_record)>2)
        {
            start_record=SyncClock::time_s();
            // output.open(_path_write.string(), ios::binary | ios::out);
            msgpack::pack(output, make_pair(header_, _buffer));
    ...
    void icvMsgpackRecorder::PlayNext(icvDataObject* data, const Uint32& source)
    {
